...
public class Pedido {
	...
	public void alterarProduto (ItemDePedido produtoNovo, int indicador){
		Produto produto = produtoNovo.getProduto();
		
				System.out.println("Deseja alterar a quantidade de produtos? ");
				...
				if (escolha == 1){
					...
					int equacao = Integer.parseInt(teclado.nextLine());
					if (equacao == 1){
						System.out.println("Digite a quantidade a incrementar ");
						int valor = Integer.parseInt(teclado.nextLine());
						if ( produto.getQuantidadeEmEstoque() > valor){
							produtoNovo.setQuantidade(valor);
							produto.naoTemMaisNoEstoque(valor);
						}
					} else if (equacao == 2){
						System.out.println("Digite a quantidade a reduzir ");
						int valor = Integer.parseInt(teclado.nextLine());
						if (produtoNovo.getQuantidade() > valor){
							valor = valor * (-1);
							produtoNovo.setQuantidade(valor);
						}
					}
     ...
     */
	public boolean incluirProduto(ItemDePedido novo, int quantidade) {

		Produto produto = novo.getProduto();

		for (int i = 0; i < MAX_PRODUTOS; i++){
			if (produtos[i].getDescricao().equals(produto)){
			System.out.println("Produto já encontrado no carrinho: " + produtos[i].getDescricao());
			alterarProduto(novo, i);
			}
		};

		if (produto.temNoEstoque(quantidade)) {
			produtos[quantProdutos++] = produto;
			produto.naoTemMaisNoEstoque(quantidade);
			return true;
		} else return false;
